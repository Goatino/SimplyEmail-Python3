...
class Conducter(object):
    ...
    def printer(self, FinalEmailList, Domain, VerifyEmail=False, NameEmails=False, output_dir="", filename=""):
        ...
        if NameEmails:
            x = 0
            if filename:
                NamePath = buildpath + "/" + filename + "_Built.txt"
            else:
            ...
            x = 0
            if filename:
                VerPath = buildpath + "/" + filename + "_Verified.txt"
            else:
            ...
            x = 0
            if filename:
                ListPath = buildpath + "/" + filename + ".txt"
            else:
    ...
    def HtmlPrinter(self, HtmlFinalEmailList, Domain, output_dir=""):
    ...
    def TaskSelector(self, domain, verbose=False, scope=False, Names=False, json="", Verify=False, output_dir="", filename=""):
        ...
        while True:
            ...
            if len(LeftOver) == 0:
                ...
                try:
                    if not json:
                        FinalCount = self.printer(FinalEmailList, domain, output_dir=output_dir, filename=filename)
                except Exception as e:
                ...
                try:
                    if json:
                        ...
                        self.HtmlPrinter(HtmlFinalEmailList, domain, output_dir=output_dir)
                except Exception as e:
        ...
        try:
            ...
            if Verify:
                ...
                if val:
                    ...
                    if VerifiedList:
                        self.printer(
                            FinalEmailList, domain, VerifyEmail=Verify, output_dir=output_dir, filename=filename)
        ...
        try:
            if Names:
                if BuiltNames:
                    self.printer(BuiltNames, domain, NameEmails=True, output_dir=output_dir, filename=filename)
        except Exception as e:
            error = " [!] Something went wrong with outputting results of Built Names:" + \
                str(e)
            print(helpers.color(error, warning=True))
        if not json:
            self.CompletedScreen(FinalCount, BuiltNameCount, domain, output_dir=output_dir)
    ...
    def TestModule(self, domain, module, verbose=False, scope=False, Names=False, json='', Verify=False, output_dir="", filename=""):
        self.logger.debug("Starting TaskSelector for: " + str(domain))
        Config = configparser.ConfigParser()
        Config.read("Common/SimplyEmail.ini")
        total_proc = int(1)
...
------------------------------------------------------------"""
    ...
    def CompletedScreen(self, FinalCount, EmailsBuilt, domain, output_dir=""):
        Config = configparser.ConfigParser()
        Config.read("Common/SimplyEmail.ini")
        ...
        if output_dir:
            Line += "   File Location: \t\t" + os.path.abspath(output_dir) + "\n"
        else:
            Line += "   File Location: \t\t" + \
        ...
        # Automatically exit without prompting for HTML report
        print(helpers.color(" [*] Results saved successfully!", status=True))
        sys.exit(0)
