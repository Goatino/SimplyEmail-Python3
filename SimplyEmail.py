...
def cli_parser():
    ...
    parser.add_argument(
        "-o", "--output", metavar='output_directory', default="", 
        help="Set this to specify output directory")
    parser.add_argument(
        "-f", "--filename", metavar='filename', default="", 
        help="Set this to specify output filename (without extension)")
    ...
    if args.h:
        parser.print_help()
        sys.exit()
    return args.all, args.e, args.l, args.t, args.s, args.n, args.verify, args.v, args.json, args.output, args.filename
...
def TaskStarter(version):
    ...
    log = helpers.log()
    log.start()
    cli_all, cli_domain, cli_list, cli_test, cli_scope, cli_names, cli_verify, cli_verbose, cli_json, cli_output, cli_filename = cli_parser()
    ...
    if cli_test:
        ...
        Task.TestModule(cli_domain, cli_test, verbose=cli_verbose,
                        ...
                        json=cli_json, output_dir=cli_output, filename=cli_filename)
    if cli_all:
